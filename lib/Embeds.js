    ...
    getInfoEmbed(id, msg) {
        return new Promise(async (fulfill, reject) => {
          try{
            ...
            // console.log(info);random 
            embed.setAuthor('nHentai | tokiharajs', this.client.nHlogo);
            embed.setColor(this.client.config.COLOR);
            // embed.setThumbnail(thumb);
            embed.setTitle(`🔍 ${res.title.pretty}`);
            embed.setDescription(`🚀 Artists: **${info.artist[0] ? info.artist.join(', ') : info.artist}**`+'\n\n🌏 Artworks Gathering:\n❯ [ https://nhentai.net/artist/'+info.artist[0].replace(' ','-').toLowerCase()
            +' ]\n❯ [ https://hitomi.la/artist/'+info.artist[0].replace(' ','%20').toLowerCase()+'-all-1.html ]'
            +'\n❯ [ https://nozomi.la/tag/'+info.artist[0].replace(' ','_').toLowerCase()+'-1.html ]'+'\n❯ [ https://www.simply-hentai.com/search?utf8=%E2%9C%93&query='+info.artist[0].replace(' ','%20').toLowerCase()+'&class_name=artist ]');
            
            embed.setURL(`https://nhentai.net/g/${res.id}`);
            embed.setImage(info.cover);
            embed.setFooter(`React for Request \n📗 == [read]\n📥 == [download]\n🗑️ == [remove] \n/ ${res.id}`.toUpperCase())
            ...
            if (info.tag[0]) embed.addField('🏷️ Tags', info.tag[0] ? info.tag.join(', ') : info.tag)
    ...
    getEmoji(id, m, msg) {
        return new Promise(async (fulfill, reject) => {
          try {
            ...
            const deleteFilter = (reaction, user) => reaction.emoji.name === `🗑️` && user.id === msg.author.id;
            ...
            forwards.on('collect', async f => {
                ...
                read.setDescription(`Artists: **${info.artist[0] ? info.artist.join(', ') : info.artist}**`);
